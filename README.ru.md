...
В моем опыте есть достойные хакатоны, [командные проекты](https://github.com/Ultramind-67) и личные ML-системы. Я стараюсь проектировать решения не только как модельные эксперименты, но и как полноценные engineering-проекты с понятной структурой, воспроизводимым запуском, измеримыми результатами и удобным интерфейсом.
...
| [Avito Split Detector](https://github.com/Ultramind-67/avito_hack_solution) | Гибридный ML-сервис для кейса Avito на IT Purple Hack 2026. Система анализирует объявление из категории ремонта и определяет, нужно ли разделить его на отдельные услуги. Проект находит дополнительные микрокатегории в тексте, принимает локальное ML-решение о сплите и при необходимости генерирует черновики новых объявлений. | Python, CatBoost, TF-IDF, rule-based detector, meta-features, FastAPI, optional OpenRouter LLM drafts, tests. Результат: 2 место в треке Avito на IT Purple Hack 2026. |
...
| [CdekStart RAG Bot](https://github.com/kzemlyanko-wq/Cdek_RAG-Bot) | RAG-бот с FastAPI backend и graph-based orchestration для ответов с учетом контекста. Система построена вокруг детерминированных retrieval-шагов и поддерживает разные LLM-провайдеры: локальные модели и внешние API. | Python, FastAPI, LangGraph, RAG, OpenAI API, Ollama, retrieval orchestration, backend API. |
